...
import {
  locateVidulumConf
} from './locate-vidulum-conf';
import {
  locateMasternodeConf
} from './locate-masternode-conf';
import {
  log
} from './logger';
...
// runDaemon function is set async // 
const runDaemon: () => Promise < ? ChildProcess > = () => new Promise(async (resolve, reject) => {
  ...
  sendToRenderer('vidulum-daemon-status', {
    error: false,
    status: 'Downloading network params, this may take some time depending on your connection speed',
  });
  ...
  if (daemonProcessId) {
    ...
    const [{
      cmd,
      pid
		}] = await findProcess('pid', daemonProcessId);
